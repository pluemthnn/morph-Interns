...
const Text:React.FC<TextProps> = ({ id, text, onDelete, moveNext, boardName }) => {
    return (
        <>
            <div className="w-full bg-pink-50 rounded-lg p-3 my-2 flex justify-center">
                {boardName === "todo" ? "" : 
                    <div className="inline-block fixed mr-72">
                        <Button name="<" id={id} moveNext={moveNext} onDelete={onDelete} />
                    </div>
                }
                ...
                <div className="inline-block fixed ml-52">
                    <Button name="X" id={id} onDelete={onDelete} moveNext={moveNext} />
                </div> 
                {boardName === "approve" ? "" :
